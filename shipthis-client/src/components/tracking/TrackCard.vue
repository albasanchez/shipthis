  ...
  <v-card class="track-card" align-self="center">
    ...
    <v-row>
        ...
        <v-row>
          <v-col cols="6" class="pb-0">
            <p class="display-5">
              ...
              <br />
              {{ ship.origin_office.place.address }}
            </p>
          </v-col>
          <v-col cols="6" class="pb-0">
            <p class="display-5" v-if="ship.destination_place">
              ...
              <br />
              {{ ship.destination_place }}
            </p>
            <p class="display-5" v-else>
              <span class="font-weight-bold"> {{ $t("tracking.to") }}: </span>
              <br />
              {{ ship.destination_office.place.address }}
            </p>
          </v-col>
        </v-row>
      </v-card-text>
      <v-col cols="12" class="checks pt-0">
        <div class="checkpoint-item">
          <v-btn
            class="mx-2 mt-1 white--text checkpoint-box"
            small
            flat
            color="primary"
          >
          ...
          <p class="checkpoint-text">
            {{ ship.origin_office.place.address }} <br />
            <span class="font-weight-light"> {{ ship.creation_date }} </span>
          </p>
        </div>
        <div
          v-for="item in ship.trajectories.check_points"
      ...
      <v-row>
        <v-col cols="4">
          <p>
            ...
            >{{ ship.person.first_name }} {{ ship.person.last_name }}
          </p>
          <p>
    ...
    <v-dialog v-model="details_dialog" max-width="650">
      <v-card class="text-center">
        ...
        <v-chip
          class="mb-2 ml-6"
          :class="ship.order_price_hist.order_type.name"
          label
        >
          {{ ship.order_price_hist.order_type.name }}
        </v-chip>
        <div class="py-0 px-6">
          <v-card-text class="py-3 my-3 form-container text-center ">
            ...
            <p>
              ...
              {{ ship.creation_date }} <br />
              <span class="font-weight-bold">
                {{ $t("newOrder.ignoreHolidays") }}:
              </span>
              {{ ship.ignore_hollydays }} <br />
              <span class="font-weight-bold">
                {{ $t("newOrder.origin") }}:
              </span>
              {{ ship.origin_office.name }} ({{
                ship.origin_office.place.address
              }})<br />
              <span class="font-weight-bold">
                {{ $t("newOrder.destiny") }}:
              </span>
              {{ ship.destination_office.name }} {{ ship.destination_place
              }}<br />
            </p>
          </v-card-text>
          <v-card-text class="py-3 my-3 form-container text-center ">
            ...
            <p>
              ...
              {{ ship.rec_document }} <br />
              <span class="font-weight-bold">
                {{ $t("labels.phoneNumber") }}:
              </span>
              {{ ship.rec_phone_code }}-{{ ship.rec_phone_number }} |
        ...
        <v-row align="center text-center">
          <v-expansion-panels class="px-9 items-accordeon" focusable hover>
            <v-expansion-panel
            ...
            >
              ...
              <v-expansion-panel-content class="mt-2">
                <p class="mb-2">
                  ...
                  {{ item.item_type_hist.base_price }} |
                  <span class="font-weight-bold">
                    {{ $t("newOrder.priceKm") }}:
                  </span>
                  {{ item.item_type_hist.price_km }} <br />
                </p>
