...
export class CharPriceHistRepository extends Repository<CharPriceHist> {
    ...
    async getCharacteristicsInfo(): Promise<CharPriceInfoDto[]>{
        ...
        const characteristicsHist: CharPriceHist[] = await this.find({ where: { ending_date: null } });
     ...
     async updateHistory(newRegister: NewCharPriceDto, char: Characteristic) : Promise<any> {

        const oldRegister: CharPriceHist = await this.findOne({ where: { ending_date: null, characteristic: char }});
