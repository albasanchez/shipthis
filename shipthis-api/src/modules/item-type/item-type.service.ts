...
export class ItemTypeService {
  ...
  async getCurrentItemPrice(): Promise<ItemPriceHist> {
    ...
    this._appLogger.log('Handling New Request: Consulting Current Item Price History Service');
  ...
  async getACurrentActiveCharacteristics(): Promise<Characteristic[]> {
    ...
    this._appLogger.log('Handling New Request: Consulting Active Characteristics Service');

    const characteristics: Characteristic[] = await this._characRepo.getAllCharacteristics();

    this._appLogger.log('Active Characteristics has been consulted sucessfully');
  ...
  async updatePriceHist(newRegister: NewItemHistDto): Promise<NewItemHistDto> {
    ...
    this._appLogger.log('Handling New Request: Update Item Price History Service');

    const newPriceHist: ItemPriceHist = await this._itemPriceRepo.updateHistory(newRegister);
  ...
  async getCharacteristicsInfo(): Promise<CharPriceInfoDto[]> {
    ...
    this._appLogger.log('Handling New Request: Consulting Characteristics Information Service');

    const characteristics: CharPriceInfoDto[] = await this._characHistRepo.getCharacteristicsInfo();

    this._appLogger.log('Characteristics Information has been consulted sucessfully');
  ...
  async updateCharHist(newRegister: NewCharPriceDto): Promise<NewCharPriceDto> {
    ...
    this._appLogger.log('Handling New Request: Update Characteristic Price History Service');

    const characteristic: Characteristic = await this._characRepo.getCharacteristicById(newRegister.id);

    if(!characteristic) {
      throw new CharacteristicNotFoundException();
    }

    const newPriceRegister = this._characHistRepo.updateHistory(newRegister, characteristic);
