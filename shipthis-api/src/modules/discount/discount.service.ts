...
import { UserNotFoundException,
         ...
         NotValidOptionException} from 'src/common/exceptions';
...
export class DiscountService {
  ...
  async getDiscountsInfo(): Promise<DiscountBasicInfoDto[]> {
    ...
    discounts.forEach(discount => {
        discountsInfo.push(
        MapperDiscount.discountToDiscountBasicInfo(discount)
      );
    });

    this._appLogger.log('Handling New Request: Discounts Consulted Successfully');
  ...
  async getDiscounts(): Promise<DiscountInfoDto[]> {
    ...
    discounts.forEach(discount => {
        discountsInfo.push(
        MapperDiscount.discountToDiscountInfo(discount)
      );
    });
  ...
  async assignDiscount(data: AssignDiscountDto) {
    ...
    await this._emailService.sendDiscountEmail(discount.name, 
      discount.percentage, 
 ...
 async createDiscount(newDiscount: UpdateDiscountDto): Promise<Discount> {
  ...
  const discount: Discount = await this._discountRepo.createDiscount(newDiscount);
...
async assignDiscounts(optionInfo: AssignDiscountsDto) {
  ...
  private async sendEmails(users: Userdata[], 
    discount: Discount) {
