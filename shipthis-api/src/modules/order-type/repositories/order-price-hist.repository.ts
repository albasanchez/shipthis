...
export class OrderPriceHistRepository extends Repository<OrderPriceHist> {
    ...
    async updateHistory(newRegister: UpdateOrderTypeHistDto, orderType: OrderType) : Promise<any> {

        const oldRegister: OrderPriceHist = await this.findOne({ where: { ending_date: null, order_type: orderType }});
