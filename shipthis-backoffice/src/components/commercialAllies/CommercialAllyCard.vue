    ...
    <v-container fluid>
      <v-row class="title-commercial-ally-info">
        <v-col cols="10" class="d-flex align-start pb-0 ">
            ...
            <h1 v-if="commercialAlly" :class="$vuetify.breakpoint.xsOnly ? 'headline' : ''" class="ml-3">{{commercialAlly.name}}</h1>
        ...
        <v-col cols="12" class="d-flex align-start pt-0 ">
            <h3 v-if="commercialAlly" :class="$vuetify.breakpoint.xsOnly ? 'body-2' : ''" class="ml-3">ID: {{commercialAlly.commercial_ally_key}}</h3>
        </v-col>
      ...
      <v-row class="pt-0 pb-0 mt-0 ">
        <v-col cols="12" class="d-flex align-center pt-0 pb-0 mt-0">
             <v-row class="pt-0 pb-0 mt-0">
              ...
              <v-col class="d-flex align-start pt-0 pb-0 mt-0">
                  <p v-if="commercialAlly" class="ml-3 pt-0 pb-0 mt-0">{{commercialAlly.manager_name + ' ' + commercialAlly.manager_last_name}}</p>
              </v-col>
            </v-row>
        </v-col>
      </v-row>
    ...
    <v-row class=" header-commercial-ally-info ">
      ...
      <v-row class="body-commercial-ally-info">
        ...
        <v-col v-if="commercialAlly" cols="12" md="1" class="d-flex justify-end align-end">
           ...
           <v-btn v-if="this.commercialAlly.status != 'DELETED'" :class="$vuetify.breakpoint.smAndDown? 'mt-3' : 'mx-2'" :fab="!$vuetify.breakpoint.smAndDown" dark small :block="$vuetify.breakpoint.smAndDown" color="indigo" @click="activateWarehouseDialog('CREATE')">
      ...
      >
        <template v-slot:item.actions="{ item }">
          <v-icon small v-if="item.status != 'DELETED'" class="mr-2" @click="activateWarehouseDialog('UPDATE',item)"
            >mdi-pencil</v-icon
          >
          <v-icon small v-if="item.status != 'DELETED'" @click="activateWarehouseDialog('DELETE',item)">mdi-delete</v-icon>
        </template>
      </v-data-table>
...
<v-row>
   <v-col class="border-p pt-0 mb-6 elevation-4 body-commercial-ally-info">
      ...
      >
        <template v-slot:item.actions="{ item }">
          <v-icon small class="mr-2" @click="viewTrackingInformation(item.id)"
            >mdi-magnify</v-icon
          >
        </template>
      </v-data-table>
    ...
    <WarehouseModal ref="warehouse_modal" @loadCommercialAllysWarehouses="loadCommercialAllysWarehouses"/>
      </v-container>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    activateWarehouseDialog(action, warehouse){
       switch (action) {
        case "CREATE":
          return this.$refs.warehouse_modal.activateWarehouseDialog(action,this.commercialAlly.commercial_ally_key);
        case "UPDATE":
          return this.$refs.warehouse_modal.activateWarehouseDialog(action,this.commercialAlly.commercial_ally_key, warehouse);
        case "DELETE":
          return this.$refs.warehouse_modal.activateWarehouseDialog(action, this.commercialAlly.commercial_ally_key, warehouse);
        default:
          break;
    ...
    async loadCommercialAllysWarehouses() {
      await CommercialAlliesRepository.getCommercialAllysWarehouses({commercial_ally_key: this.commercialAlly.commercial_ally_key})
        .then((response) => {
    ...
    async loadCommercialAllysPickups() {
      await CommercialAlliesRepository.getCommercialAllysPickups({commercial_ally_key: this.commercialAlly.commercial_ally_key})
        .then((response) => {
  ...
  async mounted() {
    this.commercialAlly = await this.$store.getters["commercialAlly/getCommercialAlly"];
