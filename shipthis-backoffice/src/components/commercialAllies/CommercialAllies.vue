          ...
          <v-card elevation="4">
            <v-card-title>
              ...
              <v-text-field
                ...
                hide-details
              ></v-text-field>
              <v-btn  :class="$vuetify.breakpoint.xsOnly? 'mt-3' : 'mx-2'" :fab="!$vuetify.breakpoint.xsOnly" dark small :block="$vuetify.breakpoint.xsOnly" color="indigo" @click="activateCommercialAllyDialog('CREATE')">
            ...
            >
              <template v-slot:item.actions="{ item }">
                <v-icon small class="mr-2" @click="viewCommercialAlly(item)"
                  >mdi-magnify</v-icon
                >
                <v-icon small v-if="item.status!='DELETED'" class="mr-2" @click="activateCommercialAllyDialog('UPDATE',item)"
                  >mdi-pencil</v-icon
                >
                <v-icon small v-if="item.status!='DELETED'" @click="activateCommercialAllyDialog('DELETE',item)">mdi-delete</v-icon>
              </template>
            </v-data-table>
          </v-card>
        </v-col>
...
export default {
  ...
  methods: {
    activateCommercialAllyDialog(action, receiver){
       switch (action) {
        case "CREATE":
          return this.$refs.commercial_ally_modal.activateCommercialAllyDialog(action);
        case "UPDATE":
          return this.$refs.commercial_ally_modal.activateCommercialAllyDialog(action, receiver);
        case "DELETE":
          return this.$refs.commercial_ally_modal.activateCommercialAllyDialog(action, receiver);
        default:
          break;
    ...
    async viewCommercialAlly(commercialAlly){
      await this.$store.dispatch("commercialAlly/setCommercialAlly", commercialAlly);
